            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table id="basic-datatable" class="table dt-responsive nowrap w-100">
                        ...
                        <tbody>
                            @foreach ($listeningQuestionBank as $index => $listening)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $listening->skill_name }}</td>
                                    <td>{{ $listening->slug }}</td>
                                    <td>{{ $listening->time_limit }}</td>
                                    ...
                                    <td>
                                        ...
                                        <form id="delete-form-{{ $listening->slug }}"
                                            action="{{ route('test.skill.destroy', $listening->slug) }}" method="POST"
                                            style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
